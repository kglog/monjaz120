...
export default function ProgrammingPage() {
  const data = CATEGORY_MAP.programming;
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-[rgb(249,251,253)] text-slate-900">
          ...
          <div className="grid gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {data.popular.map((p) => (
              <CategoryCard
                ...
                title={p.title}
                tag={p.tag}
                icon={p.title.includes("موقع") ? <Globe className="w-5 h-5" /> : p.title.includes("وورد") ? <Code className="w-5 h-5" /> : p.title.includes("تطبيق") ? <Smartphone className="w-5 h-5" /> : <Database className="w-5 h-5" />}
              />
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-14">
        ...
        <RelatedPills items={["تصميم مواقع","SEO","تحليلات","تطوير واجهات"]} />
      </section>
    </main>
  );
}
