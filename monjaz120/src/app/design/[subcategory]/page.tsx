...
import { getCategoryByKey } from "../../../lib/design/categories";
...
export default function DesignSubcategoryPage({ params }: { params: any }) {
	// Next.js may provide `params` as a Promise in some runtimes.
	// Unwrap using `React.use(params)` when available (future-proof),
	// otherwise fall back to the value directly.
	// This prevents the runtime warning about accessing params properties.
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	const resolvedParams: any = typeof React.use === "function" ? React.use(params) : params;
	const categoryKey = resolvedParams?.subcategory;
	const cat = getCategoryByKey(categoryKey) || { nameAr: categoryKey, descAr: "" };
				...
				if (mounted && json?.ok && Array.isArray(json.services)) {
					const items: Card[] = json.services
						...
						.filter((it: any, idx: number) => {
							...
							return keys.includes(categoryKey.toLowerCase()) || keys.includes(categoryKey.replace('-', '_').toLowerCase());
						});
					setServices(items);
				}
			} catch (err) {
				...
				<div className="mx-auto max-w-7xl px-4 py-10">
					...
					<nav aria-label="breadcrumb" className="mb-4 text-sm text-slate-600">
						<ol className="flex items-center gap-2">
							...
							<li className="opacity-60">/</li>
							<li className="font-semibold text-slate-700">{cat.nameAr}</li>
							<li className="opacity-60">/</li>
							<li className="font-bold text-slate-900">{cat.nameAr}</li>
						</ol>
					</nav>

					<div className="flex flex-col items-center text-center">
						<h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-slate-900">{cat.nameAr}</h1>
						<p className="mt-2 text-slate-700">{cat.descAr || 'أعمال إبداعية على يد مصممين محترفين — سعر نهائي شامل الضريبة وعمولتنا تُدار داخليًا.'}</p>
						...
						<div className="mt-6 flex w-full max-w-xl items-center gap-2">
							<div className="relative w-full">
								...
								<input
									...
									placeholder={`ابحث عن خدمة ${cat.nameAr}…`}
				...
				<div className="grid grid-cols-12 gap-6">
					...
					<div className="col-span-12 lg:col-span-9">
						<div className="mb-4 flex items-center justify-between">
							<h2 className="text-lg font-bold text-slate-900">{filtered.length} خدمة — {cat.nameAr}</h2>
